$(document).ready(function () {
    $('#metadatas div').each(function () {
        ...
        $(this)
            .find('input')
            .attr('name', 'md_' + $(this).find('input').attr('id'));
    });
...
function formatFileLabel(label) {
    ...
    var $hiddenInput = $('<input>').attr('type', 'hidden').attr('name', '');
...
function getMetadata() {
    ...
    var content = value.content[lang]
        ? value.content[lang]
        : getFirstOf(value.content);

    // Create DOM elements securely instead of HTML string concatenation
    var $div = $('<div>')
        .addClass('metadata input-group')
        .css('margin-bottom', '2px');

    var $span = $('<span>').addClass('label label-primary').css({
        'font-size': 'inherit',
        display: 'block',
        'text-align': 'justify',
        'white-space': 'normal',
    ...
    // Add hidden input
    var $hiddenInput = $('<input>').attr('type', 'hidden').attr('value', '');
	...
	*/
    ...
    if (errors.length) {
        ...
        if (!$('.modal-body .errors').length) {
            $('.modal-body').append(
                $('<div>').addClass('errors').append($errorContainer)
            );
        } else {
...
$(document).ready(function () {
    ...
    var hasPublishedArticles =
        form.data('volume-has-published-articles') === 'true';
    ...
    if (hasPublishedArticles) {
        ...
        var readonlyMessage =
            form.data('readonly-title-message') ||
            ...
            form.on('submit', function (e) {
                ...
                titleFields.each(function () {
                    ...
                    if (field.val() !== originalTitles[fieldName]) {
                        console.warn(
                            'Volume title modification attempt blocked (frontend)'
                        );
