...
const EpisciencesMailingList = (function () {
    ...
    function init(options) {
        ...
        const modalUsersTableBody = document.querySelector(
            '#modal-users-table tbody'
        );
        const selectedUsersTableBody =
        ...
        const audienceTableBody = document.querySelector(
                ...
                if (roleValue !== '') {
                    try {
                        const roles = JSON.parse(
                            row.getAttribute('data-roles') || '[]'
                        );
                        matchesRole =
                            Array.isArray(roles) &&
                            roles.some(r => String(r) === roleValue);
                    } catch (e) {
                        matchesRole = false;
                    }
                }
        ...
        if (userSearchInput)
            userSearchInput.addEventListener('keyup', filterModalUsers);
        if (roleFilterSelect)
            roleFilterSelect.addEventListener('change', filterModalUsers);
        ...
        if (audienceSearchInput) {
            audienceSearchInput.addEventListener('keyup', function () {
                ...
                const rows =
                    audienceTableBody.querySelectorAll('.audience-row');
                rows.forEach(row => {
                    const searchData = row.getAttribute('data-search') || '';
                    row.style.display =
                        searchData.indexOf(value) > -1 ? '' : 'none';
                });
            });
        }
        ...
        if (refreshPreviewBtn) {
            refreshPreviewBtn.addEventListener('click', function () {
                ...
                const uids = Array.from(
                    document.querySelectorAll('.uid-input')
                    ...
                    .then(data => {
                        ...
                        if (members.length === 0) {
                            ...
                            members.forEach(member => {
                                ...
                                tdEmail.style.cssText =
                                    'padding: 12px 20px; vertical-align: middle;';
                                const emailCode =
                                    document.createElement('code');
                                emailCode.style.cssText =
                                    'background: #f4f4f4; color: #555; padding: 2px 6px; border-radius: 4px; border: none;';
            ...
            manageForm.addEventListener('click', function (event) {
                ...
                if (removeBtn) {
                    ...
                    if (row) {
                        ...
                        const screenName = screenNameCell
                            ? screenNameCell.textContent.trim()
                            : '';
                        row.remove();
                        const remainingRows =
                            selectedUsersTableBody.querySelectorAll(
                                'tr.member-row'
                            );
                        if (remainingRows.length === 0) {
                            ...
                            emptyTd.style.cssText =
                                'padding: 40px 0; border-top: none;';
                            const emptyP = document.createElement('p');
                            emptyP.className = 'text-muted';
                            emptyP.style.cssText =
            ...
            modalBody.addEventListener('click', function (event) {
                ...
                if (addBtn) {
                    ...
                    if (!document.getElementById('user-row-' + uid)) {
                        const noUsersMsg =
                            document.getElementById('no-users-msg');
                        if (noUsersMsg) {
                            noUsersMsg.remove();
                        }
                        ...
                        const tdName = document.createElement('td');
                        tdName.style.cssText =
                            'vertical-align: middle; border-top: 1px solid #f9f9f9;';
                        const nameSpan = document.createElement('span');
                        ...
                        tdAction.style.cssText =
                            'vertical-align: middle; text-align: right; border-top: 1px solid #f9f9f9;';
                        const removeBtn = document.createElement('button');
                        removeBtn.type = 'button';
                        removeBtn.className =
                            'btn btn-link btn-xs remove-user text-danger';
                        removeBtn.setAttribute('data-uid', uid);
                        removeBtn.setAttribute(
                            'aria-label',
                            options.removeUserMsg + ' ' + screenName
                        );
