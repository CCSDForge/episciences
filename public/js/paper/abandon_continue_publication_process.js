    ...
    $('#submit-modal').on('click', function () {
        ...
        $request.done(function (response) {
            if(response && !response['error']){
                ...
                $("#message").addClass("alert alert-danger").html(response['error']);
            }
        });

        $request.fail(function (jqXHR, textStatus) {
            console.log("Une erreur interne s'est produite lors de l'appel de la fonction doAction : " + textStatus )
        });
