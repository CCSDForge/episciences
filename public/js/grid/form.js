...
function addValue (btn) 
{
	...
	var pos = $(prev).find('input[id^="position_"]').val(); pos++;
	var max = $('.sortable').children().length;
	
	var clone = $(prev).clone();
	
	$(clone).html($(clone).html().replace(/option_[0-9]+/gi, 'option_'+pos));	
	$(clone).attr('id', 'li_'+pos); // Nouvel id du <li>
	$(clone).find('button[id^="close_"]').attr('id', 'close_'+pos); // Nouvel id du <button> close
	$(clone).find('button[id^="close_"]').attr('name', 'close_'+pos); // Nouveau name du <button> close
	...
	if ($(clone).find('div[class="input-group"]').length > 1) {
		$(clone).find('div[class="input-group"]:not(:last)').each(function() {
