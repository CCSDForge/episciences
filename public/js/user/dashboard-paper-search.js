...
class DashboardPaperSearch {
    constructor(container) {
        ...
        this.identifier = container.querySelector(
            '[data-role="paper-identifier"]'
        );
        ...
        if (!this.identifier || !this.submitBtn) {
            console.error(
                'Dashboard paper search: Required elements not found in container',
                container
            );
            return;
        }
    ...
    goToArticle() {
        ...
        if (isNaN(idValue) || idValue <= 0) {
            alert(
                translate(
                    "La valeur saisie n'est pas correcte, l'identifiant doit être un nombre positif."
                )
            );
            return;
        }
        ...
        fetch('/paper/ajaxgetlastpaperid', {
            method: 'POST',
            headers: {
            ...
            .then(response => {
                try {
                    ...
                    if (lastDocid && lastDocid > 0) {
                        ...
                        alert(
                            translate("L'article avec l'identifiant : ") +
                                idValue +
                                translate(
...
document.addEventListener('DOMContentLoaded', function () {
    const searchContainers = document.querySelectorAll(
        '[data-component="dashboard-paper-search"]'
    );
