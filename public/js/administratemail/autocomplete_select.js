...
if (!(recipientsList) || !('user_'+ui.item.uid in recipientsList)) {
	...
	result += "<input type='hidden' name='to_"+ui.item.uid+"' value='"+JSON.stringify(ui.item.uid)+"'>";
	result += "<span onclick='removeRecipient(this)' class='glyphicon glyphicon-remove icon-action'></span>";
	result += "</span> ";
