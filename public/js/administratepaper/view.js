...
function getReviewerMenu(button) {
    ...
    if (
        ...
        content += '<li>';
        content +=
            '<a class="modal-opener" href="/administratepaper/updatedeadline/aid/' +
            aid +
            '" ';
        content += 'data-width="50%" ';
    ...
    if (
        ...
        content += '<li>';
        content +=
            '<a href="/administratepaper/refreshrating/id/' +
            docid +
            '/reviewer_uid/' +
            uid +
            '"  ';
...
function getDoiForm(button, docId, url = '/administratepaper/doiform') {
    ...
    request.done(function (result) {
        ...
        $('form[action^="/administratepaper/savedoi"]').on(
            'submit',
            function () {
                let sRequest = ajaxRequest(
                    '/administratepaper/savedoi',
...
function refreshPaperHistory(docid) {
    ...
    let refreshLogs = ajaxRequest('/administratepaper/displaylogs', {
        docid: docid,
    });
...
function removeDoi(button, paperId, docId, doi) {
    let removeDoi = ajaxRequest('/administratepaper/ajaxrequestremovedoi', {
        paperId: paperId,
        docId: docId,
        doi: doi,
    });
...
function removeCoAuthor(docId, uid, rvid) {
    let removeCoAuthor = ajaxRequest(
        '/administratepaper/ajaxrequestremovecoauthor',
        {
            docId: docId,
            uid: uid,
            rvid: rvid,
...
function getRevisionDeadlineForm(button, docId, commentId = null) {
    ...
    request.done(function (result) {
        ...
        $('form[action^="/administratepaper/updaterevisiondeadline"]').on(
            'submit',
            function () {
