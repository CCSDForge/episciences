$(document).ready(function () {
    ...
    var tags = [{text: translate('Code de la revue'), value: review['code']},
        ...
        {text: translate('Nom complet du rédacteur'), value: editor.full_name},
        {text: translate('E-mail du rédacteur'), value: editor.email}
    ];
    if (reviewer.user_name) {
        tags.push({text: translate('Identifiant du destinataire'), value: reviewer.user_name});
    }
    if (reviewer.screen_name) {
        tags.push({text: translate("Nom d'affichage du destinataire"), value: reviewer.screen_name});
    }
    var options = {
    ...
    $('#deadline-id').change(function () {
        ...
        if (isISOdate(deadline) &&
            ...
            msg = msg.replace(/<span class="updated_deadline">(.*?)<\/span>/, '<span class="updated_deadline">' + getLocaleDate(deadline, {language: locale, country: locale}) + '</span>');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status == 1) {
                ...
                $('#modal-box').find('.errors-message').html('* ' + response.message);
