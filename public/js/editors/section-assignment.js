...
function getEditors(button, sid) {
    ...
    let editorsFormRequest = ajaxRequest(JS_PREFIX_URL + 'section/editorsform', {sid: sid});
            ...
            if (!$(this).data('submitted')) { // to fix duplicate ajax request
                ...
                saveEditorsRequest.done(function (response) {
                    if (response == 1) {
                        ...
                        let refreshEditorsRequest = ajaxRequest(JS_PREFIX_URL + 'section/displayeditors', {sid: sid});
