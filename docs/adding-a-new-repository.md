...
| **BAOBAB (WACREN, InvenioRDM 13.1)** | `repository` | OAI-PMH 2.0 (`https://baobab.wacren.net/oai2d`, `metadataPrefix=datacite`) for the Dublin Core body + InvenioRDM REST API v1 (`/api/records/{id}`) for enrichments | DataCite Kernel 4 XML (via OAI-PMH) + InvenioRDM JSON (files, authors, access, concept id, via REST) | Authors from `person_or_org` (not `creatorName`, which InvenioRDM writes "Family, Given"), mirrored files via `links.content` (`FilesEnrichmentInterface`), related datasets filtered for community memberships (`LinkedDataEnrichmentInterface`), concept identifier from `parent.id` (`ConceptIdentifierInterface`), ARK resolution via a PID search query | `Episciences_Repositories_BAOBAB_Hooks` |
...
BAOBAB's OAI-PMH endpoint used to answer `Identify`/`ListMetadataFormats`/
`ListSets` correctly but return HTTP 500 (as an HTML error page, not an
OAI-PMH error document) on every verb that must emit at least one record —
`GetRecord`/`ListIdentifiers`/`ListRecords` alike. Until this was fixed
(WACREN upgraded their InvenioRDM instance; re-verified working 2026-09-08),
the workaround was not instance-specific: InvenioRDM exposes the same metadata
serializers on its REST API through content negotiation, so
`GET /api/records/{id}` with `Accept: application/vnd.datacite.datacite+xml`
returns the same DataCite XML that OAI-PMH would otherwise serve, without
touching the broken endpoint at all. If another InvenioRDM-based repository
shows the same symptom, the same workaround applies: fetch metadata over the
REST API instead of OAI-PMH (keep `metadata_sources.base_url` `NULL` so the
platform never falls back to the broken OAI endpoint via
`Episciences_Submit::loadRecord()`), and compile `PAPERS.RECORD` from DataCite
rather than from InvenioRDM's `oai_dc` (which double-escapes HTML in
`dc:description` on at least this instance). Once/if the instance's OAI-PMH is
fixed, `metadata_sources.base_url` can be set to the real endpoint and the
hook's DataCite fetch switched from REST content negotiation to
`Episciences_Repositories_Common::getRecord()` with `metadataPrefix=datacite`,
as done for BAOBAB.
