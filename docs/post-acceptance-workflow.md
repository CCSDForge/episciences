...
PR #1083 adds a second, opt-in post-acceptance state machine (statuses 34–39) that is functionally close to the 18–24/26/28/32/33 graph above — see the code-review notes in `tmp/pr1083.md` (§2.5) for the detailed status-by-status mapping and the long-term maintenance implications of running two parallel pipelines.
