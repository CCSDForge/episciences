...
|----------|-------------|
| [Developer Setup](./docs/dev-setup.md) | Docker-based local environment setup, services, credentials, and troubleshooting |
| [Console Commands](./docs/console-commands.md) | Full reference for all `scripts/console.php` CLI commands |
| [Paper Statuses](./docs/paper-statuses.md) | All paper status codes and constants defined in `Episciences_Paper` |
| [Log Event Types](./docs/log-event-types.md) | All log event codes tracked by `Episciences_Paper_Logger` |
| [KPI Downloads Format](./docs/kpi-downloads-format.md) | JSON schema for `data/kpi_downloads.json` generated by `stats:download-kpi` |
| [Statistics Pipeline](./docs/STATISTICS_UPDATE_README.md) | Full stats pipeline: Apache logs → `STAT_TEMP` → `PAPER_STAT` → KPI JSON |
