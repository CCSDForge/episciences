...
describe('checkDataverse function', () => {
  ...
  test('should use custom translate function', async () => {
    ...
    expect(customTranslate).toHaveBeenCalledWith('Proposer un jeu de données');
    expect(result.submitEntryText).toBe('TRANSLATED: Proposer un jeu de données');
  });
  ...
  test('should send correct FormData with repository ID', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/submit/ajaxisdataverse', expect.objectContaining({
      method: 'POST',
      headers: {
        'X-Requested-With': 'XMLHttpRequest'
      },
  ...
  test('should handle fetch response errors', async () => {
    ...
    const mockFetch = jest.fn().mockResolvedValue({
      text: jest.fn().mockRejectedValue(new Error('Response parsing error'))
    });
