...
function removeVersionFromIdentifier(identifier, options = {}) {
  ...
  const versionField = mockVersionField || mockGetElementById('search_doc-version');
  ...
  try {
    ...
    if (isDataverseRepo || url.search) {
      ...
      if (url.hostname === 'arxiv.org' && url.pathname.includes('/abs/')) {
        ...
        // Handles both new format (e.g., 2310.02192) and old format (e.g., hep-th/9704188, alg-geom/9202002)
    ...
    if (versionFromUrl) {
      ...
      return removeVersionFromIdentifier(identifier, { mockVersionField });
    }
  } catch (error) {
...
function setPlaceholder(options = {}) {
  ...
  const searchDocDocId = mockDocIdField || mockGetElementById("search_doc-docId");
  const searchDocRepoId = mockRepoIdField || mockGetElementById("search_doc-repoId");
