...
- Add reviewer statistics dashboards (`Episciences\Reviewer\StatsQuery`), linked from the user dashboard:
  - `/reviewersstats` for the journal managers: one row per reviewer (grouped by e-mail across registered and account-less invitations) with pending invitations, reviews in progress, completed and overdue reviews, maximum delay, reminders sent, last invitation and report dates, response and acceptance rates, and average review time. Sortable, paginated, limited to a rolling history of 24 months at most (GDPR), and filterable by name/e-mail (with reviewer suggestions opening the reviewer's page), period, pending invitations, reviews in progress, overdue reviews and "only the papers I am responsible for". Guest editors, and editors when `encapsulateEditors` is on, only see the reviewers they invited or assigned, and those of the papers they are currently assigned to. Nobody sees the reviewers of the papers they submitted or co-author and, when conflicts of interest are enabled, editorial staff only see the papers they confirmed having no conflict with (administrators without an editorial role: every paper except those they declared a conflict with).
  - A per-reviewer detail page: the reviewer's account (shared profile card) and every invitation grouped by paper and version, with replaced attempts hidden behind a toggle.
  - `/myreviewstats` for reviewers: their own statistics in the journal, over their whole history.
  - Data-model pitfalls behind these numbers (stale statuses, invitation rows inserted on every state change, reports not tied to an assignment attempt) are documented in `docs/reviewer-assignments-data-model.md`.
- Add a shared user profile card (`user/user_profile.phtml`, view model `Episciences\User\ProfileCard`), used by `/user/view`, the "My account" dashboard panel and the reviewer statistics detail page: avatar (photo, else initials), ORCID, affiliations, web sites, social media and roles in the journal; e-mail, language, registration date and biography are only shown to the person themself and to the secretaries, chief editors, administrators and (unless `encapsulateEditors` is on) editors of a journal they hold a role in; never to guest editors.
- Open a user's profile card in a dialog from `/user/list` (address-card icon), loaded from the new `/user/card` action (journal managers only); a modified click, or a failed request, still opens `/user/view`.
...
- Update dependencies.
- `/user/view` and the "My account" dashboard panel now use the shared profile card: the ORCID iD is shown in full and users without a photo get their initials instead of a placeholder.
- Add an `AID_SENDING_DATE` index on `USER_INVITATION` (`AID`, `SENDING_DATE`), the join column to `USER_ASSIGNMENT`: run `src/mysql/2026-07-12-add-user-invitation-aid-index.sql` when deploying.
