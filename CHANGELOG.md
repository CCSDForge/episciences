...
- [353](https://github.com/CCSDForge/episciences/issues/353): fix doi queue not saved + fix delete volume (alter the table for volume doi queue).
## 1.0.40.7 - 2023-06-08
### Fixed
...
- Feature to reset the API's password
- Statistics indicators: added acceptance rate by year
- Enrichment of authors via HAL TEI using the metadata update button (get OpenAire Metadatas and HAL TEI Metadatas)
- [#181](https://github.com/CCSDForge/episciences/issues/181): Administrator are able to add co-authors (using real
  accounts only) to submissions. They will receive email notifications when the paper status changes. Social contacts
  are available too for co-authors.
- Volume creation or modification: the volume can be tagged for publishing proceedings of a conference

## 1.0.40.5 - 2023-06-01

### Fixed

- Fixed: [#352](https://github.com/CCSDForge/episciences/issues/352) Augmenter la taille du champ adresse Mastodon / Increase the size of the Mastodon address field
...
- Updates on label and download button
  size [#342 Feature request: "Consult the article webpage"](https://github.com/CCSDForge/episciences/issues/342)
...
- [278](https://github.com/CCSDForge/episciences/issues/278): In case an article has been refused: new option to allow
...
- Cancel a DOI assignment for the journal. The feature is available only with manual DOI assignment setting and until
  the DOI has been requested to Crossref.
- The user profile email update form will detect duplicate accounts and allow you to request merging duplicate accounts
- [283](https://github.com/CCSDForge/episciences/issues/283): new template pages for journal websites (credits,
  publishing-policies, ethical-charter)
- Automated Metadata Enrichment: for authors via HAL XML-TEI when using the metadata update button (OpenAire and HAL
  Metadata will automatically update)
- [95](https://github.com/CCSDForge/episciences/issues/95): Twitter and Mastodon support through user profiles.
  Possibility to share published papers via admin paper page. The feature will automatically mention the @user if they
  have added their username in their profile
- Journal may use a specific email alias for bounced emails sent by the platform. Using the new email alias requires a
  request to Episciences support team for setting up the recipients of the alias.

### Added for upcoming features

- COAR Inbox notifications: new script to initialize submissions, pending implementation on HAL
- It is now possible to use LemonLDAP::NG authentication. Pending the release of the new CCSD LemonLDAP::NG in
  production.

## 1.0.39.15 - 2023-05-22

- Fixed [RT#186373]: in some situations, an article can be accepted several times. Now, the first date of acceptance
...
- related to Allow post - accepting article revisions: "Contact without sending a new version" is now disabled when
  answer final version request
...
- [#325](https://github.com/CCSDForge/episciences/issues/325): the system allows accepting an invitation after it has
  been canceled
...
- [#316](https://github.com/CCSDForge/episciences/issues/316): it is no longer necessary to validate / create an account
  before declining the invitation.

### Fixed

- 'Not enough reviewers - editor copy: editors' reminder: editors received the reminders before the deadline set in the
...
- Added three options for sharing the paper password (arXiv):
    ...
    * Required: sharing is required when submitting a new version and responding to a revision request without any
      changes
...
- [#212](https://github.com/CCSDForge/episciences/issues/212): There is a difference between the capitalization of the
  first and last names in the user's profile
...
- [#280](https://github.com/CCSDForge/episciences/issues/280): set spellcheck to false in dynamic datatable search
  boxes.
- [#281](https://github.com/CCSDForge/episciences/issues/281): Submit an article > Guidelines section: harmonization of
  journal/review terms [FR]
...
- Incorrect paper status LMCS #10145 (related
  to https://github.com/CCSDForge/episciences/blob/main/CHANGELOG.md#10273---2022-03-18)
...
- Fixed [#RT169943]: the tag %%SENDER_SCREEN_NAME%% (obsolete) that should have contained the editor's screen name has
  not been replaced:
...
- Article managers (authors, editors, etc.) may add new optional - but recommended - metadata to the documents
    ...
    - Affiliations: for affiliations the [ROR](https://ror.org/) or a free text may be used. If an institution is not
      available in the ROR, a
      simple text entry may be used.
- Automatic enrichment of metadata:
    - Licenses: retrieved
      from [Datacite](https://datacite.org/), [OpenAIRE Research Graph](https://graph.openaire.eu/), [HAL](https://hal.archives-ouvertes.fr/)
    - ORCID: retrieved from [OpenAIRE Research Graph](https://graph.openaire.eu/)
    - Linked Datasets: [Scholexplorer](https://scholexplorer.openaire.eu/), [HAL](https://hal.archives-ouvertes.fr/):
      Datasets linked to Episciences
      publications
    - Funding: Research projects from [OpenAIRE Research Graph](https://graph.openaire.eu/)
      and [HAL](https://hal.archives-ouvertes.fr/)
...
- Exports formats:
    ...
    - The new collected metadata have been added to Datacite, [Crossref](https://www.crossref.org/) and DOAJ export
      formats
