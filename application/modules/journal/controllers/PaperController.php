...
class PaperController extends PaperDefaultController
{
    ...
    public function pdfAction()
    {
        ...
        if ($paper && $paper->getRvid() == RVID && $paper->getRepoid() != 0) {
            ...
            try {
                ...
                if (!$paper->isPublished()) { // Redirection vers l'article publié
                ...
                if (Episciences_Auth::getUid() != $paper->getUid()) {
                    Episciences_Paper_Visits::add($paper->getDocid(), 'file');
                }

                $handler = curl_init($paper->getPaperUrl());
                curl_setopt($handler, CURLOPT_USERAGENT, DOMAIN);
                curl_setopt($handler, CURLOPT_FOLLOWLOCATION, true);
                $pdf = curl_exec($handler);
                curl_close($handler);
    ...
    public function viewAction()
    {
        ...
        $settings = array(self::TYPES_STR => array(
    ...
    private function save_contributor_answer(int $id, Episciences_Paper $paper, int $commentType = Episciences_CommentsManager::TYPE_INFO_ANSWER): bool
    {
        ...
        // paper status update  *******************************************
        $settings = array('unanswered' => true, 'type' => Episciences_CommentsManager::TYPE_INFO_REQUEST);
        ...
        if ($reviewer->findWithCAS($oComment->getUid())) {
            ...
            return
                Episciences_Mail_Send::sendMailFromReview(
                    $reviewer, $paper, Episciences_Mail_TemplatesManager::TYPE_PAPER_COMMENT_ANSWER_REVIEWER_COPY,
                    $reviewerTags, Episciences_Auth::getUid(), $attachmentsFiles, true
                ) &&
    ...
    public function saveanswerAction()
    {
        ...
        if (!empty($post['comment']) && $request->isPost()) {
            ...
            foreach ($recipients as $recipient) {
                ...
                Episciences_Mail_Send::sendMailFromReview($recipient, $paper, Episciences_Mail_TemplatesManager::TYPE_PAPER_REVISION_ANSWER, $tags, Episciences_Auth::getUid());
            }
    ...
    private function reassignReviewers($reviewers, Episciences_Paper $paper1, Episciences_Paper $paper2, $submissionType, Episciences_User $sender = null)
    {
        ...
        $paper1_url = $this->view->url(array(
        ...
        $paper2_url = $this->view->url(array(
            self::CONTROLLER => self::CONTROLLER_NAME,
    ...
    private function reinviteReviewers($reviewers, Episciences_Paper $paper1, Episciences_Paper $paper2, Episciences_User $sender = null)
    {
        ...
        foreach ($reviewers as $reviewer) {
            ...
            $invitation_url = $this->view->url(array(
    ...
    public function newversionAction()
    {
        ...
        // load form
        $form = Episciences_Submit::getNewVersionForm(array('newVersionOf' => $paper->getDocid()), $defaults);
    ...
    public function submittedAction()
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            $settings = array(
                'is' => array('uid' => Episciences_Auth::getUid()) + Episciences_PapersManager::getFiltersParams(),
                'isNot' => array(self::STATUS => array(Episciences_Paper::STATUS_OBSOLETE, Episciences_Paper::STATUS_DELETED)),
                'limit' => $limit,
                'offset' => $offset
            );
    ...
    public function ratingsAction()
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            $settings = array(
                'is' => array('rvid' => RVID) + Episciences_PapersManager::getFiltersParams(),
                'limit' => $limit,
                '$offset' => $offset
            );
    ...
    private function save_reviewer_comment(Episciences_Paper $paper)
    {
        ...
        return
            (
                Episciences_Mail_Send::sendMailFromReview(
                    $contributor, $paper,
                    Episciences_Mail_TemplatesManager::TYPE_PAPER_COMMENT_FROM_REVIEWER_TO_CONTRIBUTOR_AUTHOR_COPY,
                    $contributorTags, Episciences_Auth::getUid(), $attachmentFiles, true
                ) &&
    ...
    public function removeAction()
    {
        ...
        $authorizedStatus = array(Episciences_Paper::STATUS_SUBMITTED);
        $paper = Episciences_PapersManager::get($docId);
        ...
        Episciences_Mail_Send::sendMailFromReview($contributor, $paper, Episciences_Mail_TemplatesManager::TYPE_PAPER_DELETED_AUTHOR_COPY, $authorTags);
        ...
        foreach ($reviewers as $reviewer) {
            ...
            Episciences_Mail_Send::sendMailFromReview($reviewer, $paper, Episciences_Mail_TemplatesManager::TYPE_PAPER_DELETED_REVIEWER_COPY, $commonTags + $rTags);
        }
        ...
        if (!$isReviewer || $reviewerUid === Episciences_Auth::getUid()) { // Not reviewer or add rating
            if ($paper->getEditor(Episciences_Auth::getUid()) || Episciences_Auth:: isAllowedToUploadPaperReport()) {
                ...
                if (array_key_exists(Episciences_Auth::getUid(), $invitations)) {
                    ...
                    $oLastInvitation = Episciences_User_InvitationsManager::find(array('ID' => $lastArrayInvitation['INVITATION_ID']));
    ...
    private function informRecipient(Episciences_User $recipient, Episciences_Paper $paper, string $templateType, int $lastStatus = 0, array $CC = []): bool
    {
        ...
        return Episciences_Mail_Send::sendMailFromReview($recipient, $paper, $templateType, $tags, null, [], false, $CC );
    }
    ...
    private function saveEpisciencesUserComment(Episciences_Paper $paper, int $commentType = Episciences_CommentsManager::TYPE_INFO_REQUEST, int $parentCommentId = 0): Episciences_Comment
    {
        ...
        return !$result ? empty($result) : $comment;
    }
     ...
     * @throws Zend_Session_Exception
     */
    private function notifyAuthorAndManagersPaper(Episciences_Paper $paper, Episciences_Comment $commentRequest, Episciences_Comment $commentAnswer, string $templateAuthorType, $templateEditorType)
    {
        $requester = new Episciences_User();
        $requester->find($commentRequest->getUid());
        $commentFiles = Episciences_Tools::isJson($commentAnswer->getFile()) ? json_decode($commentAnswer->getFile()) : (array)$commentAnswer->getFile();
        ...
        foreach ($recipients as $recipient) {
            ...
            Episciences_Mail_Send::sendMailFromReview($recipient, $paper, $templateEditorType, $adminTags, null, $attachmentsMail, false, $CC);
            //reset $CC
            $CC = [];
        }
    }
        ...
        foreach ($invitationsByStatus as $invitations) {
            foreach ($invitations as $invitation) {
                ...
                if (!$user) {
                    ...
                    Episciences_Mail_Send::sendMailFromReview($user, $paper, Episciences_Mail_TemplatesManager::TYPE_PAPER_ABANDON_PUBLICATION_REVIEWER_REMOVAL, $tags);
                }
            }
        }
    }
     ...
     * @throws Zend_Session_Exception
     */
    private function informContributor(Episciences_Paper $paper, string $templateType, array $attachments = [] , array $additionalTags = [], int $senderUid = null ){
        ...
        return Episciences_Mail_Send::sendMailFromReview($contributor, $paper, $templateType, $tags, $senderUid, $attachments);
    }
    ...
    private function completedRatingSendNotification(Episciences_Rating_Report $report, Episciences_Paper $paper)
    {
        ...
        if ($report->isCompleted()) {
            ...
            Episciences_Mail_Send::sendMailFromReview($user, $paper, Episciences_Mail_TemplatesManager::TYPE_PAPER_REVIEWED_REVIEWER_COPY, $reviewerTags, Episciences_Auth::getUid());
            ...
            foreach ($recipients as $recipient) {
                ...
                Episciences_Mail_Send::sendMailFromReview($recipient, $paper, Episciences_Mail_TemplatesManager::TYPE_PAPER_REVIEWED_EDITOR_COPY, $editorTags, Episciences_Auth::getUid(), $attachments,false, $CC);
                // reset CC
                $CC = [];
            }
        }
