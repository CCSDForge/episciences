...
class AdministratepaperController extends PaperDefaultController
{
    ...
    public function viewAction(): void
    {
        ...
        if ($commService->canAuthorContactEditors()) {
            ...
            if ($paper->getEditor(Episciences_Auth::getUid())) {
                // Handle editor main form submission (new message to author)
                $isMainPost = $request->isPost() &&
                              $request->getPost('postComment') !== null &&
                              $request->getPost('reply_to_pcid') === null;

                if ($isMainPost) {
                    $this->processEditorMainMessageSubmission($paper, $commService);
                }

                // Create main form for editor to initiate contact with author
                if (!$isMainPost) {
                ...
                if (!empty($authorToEditorComments)) {
                    ...
                    if ($isReplyPost) {
                        $this->processEditorReplySubmission($paper, (int)$request->getPost('reply_to_pcid'), $commService);
                    }
     ...
     * @return void
     */
    private function processEditorReplySubmission(Episciences_Paper $paper, int $pcid, Episciences_Paper_AuthorEditorCommunicationService $service): void
    {
        ...
        $result = $service->processReplyMessage(
            $postData,
            $pcid,
            fn() => (bool)$paper->getEditor(Episciences_Auth::getUid())
        );

        $this->handleEditorSubmissionResult($result, $paper);
    ...
    /**
     * Process editor main message submission (new message to author, not a reply)
     *
     * @param Episciences_Paper $paper
     * @param Episciences_Paper_AuthorEditorCommunicationService $service
     * @return void
     */
    private function processEditorMainMessageSubmission(Episciences_Paper $paper, Episciences_Paper_AuthorEditorCommunicationService $service): void
    {
        $postData = $this->getRequest()->getPost();
    ...
    /**
     * Handle the result of an editor message submission
     *
     * @param Episciences_Paper_MessageSubmissionResult $result
     * @param Episciences_Paper $paper
     * @return void
     */
    private function handleEditorSubmissionResult(Episciences_Paper_MessageSubmissionResult $result, Episciences_Paper $paper): void
    {
