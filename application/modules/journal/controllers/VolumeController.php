    ...
    public function editAction(): void
    {
        ...
        if ($request->isPost() && array_key_exists('submit', $request->getPost())) {
            ...
            if ($form->isValid($post)) {
                // Prevent title changes if volume has published papers (#780)
                if ($hasPublishedPapers) {
                    // Restore original title values to prevent modification
                    $originalTitles = $volume->getTitles();
                    $languages = Episciences_Tools::getLanguages();
                    foreach ($languages as $languageCode => $language) {
                        $titleKey = Episciences_Volume::VOLUME_PREFIX_TITLE . $languageCode;
