...
class Episciences_CommentsManager
{
    ...
    public static $_typeLabel = array(
        ...
        self::TYPE_EDITOR_MONITORING_REFUSED    =>  "souhait de ne plus gérer l'article",
        self::TYPE_EDITOR_COMMENT => "Commentaires des rédacteurs",
    ...
    public static function getCopyEditingReplyForms($comments, Episciences_Paper $paper)
    {
        ...
        foreach ($comments as $id => $comment) {
            ...
            if (in_array($commentType, self::$_copyEditingFinalVersionRequest)) {// load form
                ...
                $defaults = $paper->getRepoid() ? ['docId' => $paper->getIdentifier(), 'version' => $paper->getVersion(), 'repoId' => $paper->getRepoid()] : [];
                // load form
    ...
    {
        return $db->select()->from(T_PAPER_COMMENTS)->where(T_PAPER_COMMENTS . '.DOCID = ? ', $docId);
    }
