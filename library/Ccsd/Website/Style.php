...
class Ccsd_Website_Style
{
    ...
    public function save($params = [])
    {
        ...
        foreach ($this->_tags as $tag => $value) {
            ...
            $db->insert(self::TABLE, [$this->_fieldSID => $this->_sid, 'SETTING' => $tag, 'VALUE' => $value]);
    ...
    public function setStyles($params)
    {
        foreach ($params as $key => $value) {
            if (in_array($key, ['module', 'action', 'controller', 'MAX_FILE_SIZE', 'save'])) {
                continue;
            }
    ...
    public function saveCss()
    {
        ...
        if ($this->_dirname != '') {
            if (!is_dir($this->_dirname)) {
                if (!mkdir($concurrentDirectory = $this->_dirname, 0777, true) && !is_dir($concurrentDirectory)) {
    ...
    public function getCssFile()
    {
        ...
        if (isset($this->_tags['type'])) {
            if ($this->_tags['type'] == self::TYPE_TPL && isset($this->_tags['template'])) {
                ...
                $cssFile = PREFIX_URL . 'public/style.css?' . filemtime($this->_dirname . 'style.css');
            }
        }
        return $cssFile;
    }
