...
class UpdatePapers extends JournalScript
{
    ...
    private function process_single_paper($params)
    {
        ...
        if (!$this->isDebug()) {
            ...
            // if HAL and production stage, update paper metadata in repository
            $this->updateHalMetadata($paper);
            ...
            if ($update) {
                ...
                // else, get paper param and merge it with input params
                    ...
                    if (method_exists($paper, $method)) {
                        ...
                        $this->displayTrace($param . ' param has been set to: ' . $paper->$method());
                    }
                }
            }
        }
    ...
    private function updateHalMetadata(Episciences_Paper $paper)
    {
        if ($paper->getRepoid() == Episciences_Repositories::getRepoIdByLabel('Hal') && APPLICATION_ENV == 'production') {
            $this->displayInfo("updating HAL metadata");
            $paper->updateHALMetadata($this->getParam('publication_date'), null);
            $this->displayInfo("Updated HAL metadata");
        }
    }
    ...
    {
        if ($this->isDebug()) {
            $this->displayInfo("trying to retrieve publication date");
        }

        // try to set if from script parameter (csv or arg)
        if (array_key_exists('publication_date', $params)) {
            $publication_date = Episciences_Tools::getValidSQLDateTime($params['publication_date']);
        ...
        // check current paper publication date
        if ($paper->getPublication_date()) {
            $publication_date = Episciences_Tools::getValidSQLDateTime($paper->getPublication_date());
            if (Episciences_Tools::isValidSQLDateTime($publication_date)) {
                if ($this->isDebug()) {
        ...
        $dc_date = Ccsd_Tools::xpath($paper->getMetadata('record'), '//dc:date');
        if ($dc_date) {
            if (is_array($dc_date)) {
                $dc_date = array_shift($dc_date);
            }
            $publication_date = Episciences_Tools::getValidSQLDateTime($dc_date);
            if (Episciences_Tools::isValidSQLDateTime($publication_date)) {
                if ($this->isDebug()) {
                    $this->displayInfo("publication date retrieved from xml record");
                }
                return $publication_date;
            }
        }

        // set it from current date
        if ($this->isDebug()) {
            $this->displayInfo("publication date retrieved from today's date");
        }
    ...
    {
        // fetch chief editors
        $chiefRedactors = $this->_review->getChiefEditors();
